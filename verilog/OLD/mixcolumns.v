module mixcolumns (
    ...
    generate
        for (i = 0; i < 4; i = i + 1) begin : col_loop
            // 列の4バイトを抽出 (a0-a3)
 //           wire [7:0] a0 = state_in[i*32 + 0  +: 8];
 //           wire [7:0] a1 = state_in[i*32 + 8  +: 8];
 //           wire [7:0] a2 = state_in[i*32 + 16 +: 8];
 //           wire [7:0] a3 = state_in[i*32 + 24 +: 8];
...
//            assign state_out[i*32 + 0  +: 8] = xtime(a0) ^ (xtime(a1) ^ a1) ^ a2 ^ a3;
//            assign state_out[i*32 + 8  +: 8] = a0 ^ xtime(a1) ^ (xtime(a2) ^ a2) ^ a3;
//            assign state_out[i*32 + 16 +: 8] = a0 ^ a1 ^ xtime(a2) ^ (xtime(a3) ^ a3);
//            assign state_out[i*32 + 24 +: 8] = (xtime(a0) ^ a0) ^ a1 ^ a2 ^ xtime(a3);
        end
    endgenerate
